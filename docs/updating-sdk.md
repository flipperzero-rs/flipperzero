...
From the root of the repository, to build the binding for the branch/tag `0.99.1` of the official SDK:

```shell
docker run --rm $(docker build --build-arg BRANCH=0.99.1 -q -f tools/Dockerfile .) > crates/sys/src/bindings.rs
```
