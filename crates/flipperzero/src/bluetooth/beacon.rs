        ...
        let config = if let Some(config) = existing_config {
            ...
            *config
            ...
            let config = sys::GapExtraBeaconConfig {
                ...
                adv_channel_map: sys::GapAdvChannelMap_GapAdvChannelMapAll,
                adv_power_level: sys::GapAdvPowerLevelInd_GapAdvPowerLevel_0dBm,
                address_type: sys::GapAddressType_GapAddressTypeRandom,
                address,
            };
    ...
    pub fn set_address(&mut self, address: Address) -> Result<(), Error> {
        match address {
            Address::Public(address) => {
                self.config.address_type = sys::GapAddressType_GapAddressTypePublic;
                self.config.address = address;
            }
            Address::RandomStatic(mut address) => {
                address[5] &= 0b1100_0000;
                self.config.address_type = sys::GapAddressType_GapAddressTypeRandom;
                self.config.address = address;
            }
        }
...
impl Address {
    fn from_config(config: &sys::GapExtraBeaconConfig) -> Option<Self> {
        match config.address_type {
            sys::GapAddressType_GapAddressTypePublic => Some(Address::Public(config.address)),
            sys::GapAddressType_GapAddressTypeRandom => Some(Address::RandomStatic(config.address)),
            _ => None,
        }
    }
}
